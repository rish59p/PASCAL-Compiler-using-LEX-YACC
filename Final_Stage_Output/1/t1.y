...
assignment_stmt: IDENTIFIER ASSIGN expr {
	...
	| IDENTIFIER LSPAREN term RSPAREN ASSIGN expr  {
		...
		if(strcmp(symbol_table[idx].arr_tp,$<t.type>6)==0){
			if(strcmp(symbol_table[idx].arr_tp,"integer")==0)symbol_table[idx].vi.intArray[arr_ind] = $<t.v.ival>6;
			else if(strcmp(symbol_table[idx].arr_tp,"real")==0)symbol_table[idx].vi.floatArray[arr_ind] = $<t.v.fval>6;
			else if(strcmp(symbol_table[idx].arr_tp,"char")==0)symbol_table[idx].vi.intArray[arr_ind] = $<t.v.cval>6;
			else if(strcmp(symbol_table[idx].arr_tp,"boolean")==0)symbol_table[idx].vi.booleanArray[arr_ind] = strdup($<t.v.sval>6);
		}
		else{
...
factor: LPAREN expr RPAREN {
       ...
       | NOT factor {
       		...
       		if(strcmp($<t.type>2,"boolean")==0){
     			//$<t.type>$=strdup("boolean");
     			if(strcmp($<t.v.sval>2,"true")==0)$<t.v.sval>$ = strdup("false");
     			else $<t.v.sval>$ = strdup("true");
     		}
     		else {printf("ERROR : type mismatch for 'NOT' relop.\n");}
       }
	;
const: INTVAL  {$<t.type>$=$<t.type>1;$<t.v.ival>$=$<t.v.ival>1;}
     | REALVAL {$<t.type>$=$<t.type>1;$<t.v.fval>$=$<t.v.fval>1;}
     | BOOLVAL {$<t.type>$=$<t.type>1;$<t.v.sval>$=$<t.v.sval>1;}
     | CHARVAL {$<t.type>$=$<t.type>1;$<t.v.cval>$=$<t.v.cval>1;}
     ;
...
conditional_stmt:  IF  expr {ifunc($<t.type>2,$<t.v.sval>2);} THEN block_stmt if_st
		;
			
if_st: ELSE {if(execute==0)execute=1;else execute=0;} block_stmt {execute=1;}
   | {execute=1;}
   ;
	
loop: WHILE expr {wfunc($<t.type>2,$<t.v.sval>2);} DO PROG_BEGIN statement_list END
    | FOR IDENTIFIER ASSIGN expr {is_int($<t.type>4);} TO expr DO PROG_BEGIN statement_list END 
    | FOR IDENTIFIER ASSIGN expr {is_int($<t.type>4);} DOWNTO expr DO PROG_BEGIN statement_list END 
    ;
